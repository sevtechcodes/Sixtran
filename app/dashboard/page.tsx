...
export default function Page (): React.ReactElement {
        ...
        const getInitialData = async () => {
          ...
          try {
            ...
            // Assuming you want to get connectors for the selected group (first group by default)
						 if (selectedGroup) {
              const connectorsData = await getConnectors(selectedGroup, fivetranApiKey, fivetranApiSecret);
  ...
  async function handleSelect (event: ChangeEvent<HTMLSelectElement>) {
    ...
    if (group && credentials) { //added this line to make sure there are groupt
  ...
  async function pauseConnectors (connectorsToModify: Connector[]) {
		 if (credentials ) { //checking if credenntials are there
