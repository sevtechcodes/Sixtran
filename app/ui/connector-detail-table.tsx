  ...
  useEffect(() => {
    ...
    for (const table in schema.tables) {
      const filteredQueries = queries.filter(
        (q) =>
          q.table_id === table &&
          compareAsc(new Date(q?.creation_time.value), timeFrame) >= 0
      );
      ...
      const tableData: Table = {
        ...
        total_rows: queries
          .filter((q) => q.table_id === table)
          .reduce((_, q) => q.total_rows, 0),

        select_queries: filteredQueries.filter(
          (q) => q.statement_type === 'SELECT'
        ).length,
        merge_queries: filteredQueries.filter(
          (q) => q.statement_type === 'MERGE'
        ).length,
        ...
        mar: filteredQueries.reduce(
          (acc, q) => acc + q.inserted_rows + q.updated_rows + q.deleted_rows,
          0
        ),
      };
      data.push(tableData);
    ...
    <>
      <div className='p-4'>
        <div className='flex items-center'>
          ...
          />
          <h1 className='font-bold text-lg mr-3 ml-3'>Dataset:</h1>
          <h1 className='bg-[#06AB78] text-white rounded px-2 py-1 text-lg font-bold'>
      ...
      <div>
        ...
        >
          {TIMEFFRAMES.map((timeframe, index) => {
            if (timeframe === 30) {
              return (
                <option key={index} value={timeframe} selected>
                  {formatDuration({ days: timeframe })}
                </option>
              );
            } else {
              return (
